...
// - Performance mode (default): DATA is fire-and-forget (no per-packet ACK wait)
// - Teardown remains reliable: FIN is retransmitted until ACKed by all receivers
...
//
// NOTE (benchmarking):
// - Handshake still waits for SYN|ACK to establish the cohort.
// - DATA is sent without waiting for ACKs (fire-and-forget) unless --wait-acks is used.
// - FIN always waits for ACKs from the whole cohort.
...
struct Args {
    ...
    // Performance mode:
    //  - If wait_data_acks == false (default), we do NOT wait for ACKs after each DATA send (fire-and-forget).
    //  - Handshake still requires SYN|ACK replies to establish the cohort.
    //  - FIN always waits for ACKs (to match TCP-like teardown behavior), regardless of this flag.
    bool wait_data_acks = false;
};
...
static void usage(const char* prog) {
    std::cerr << "Usage: " << prog << " --group A.B.C.D --port P --sender-port S --expected N [--file path]"
              ...
              << " [--wait-acks|--no-wait-acks]\n";
    std::cerr << "\nNotes:\n"
              << "  * DATA is fire-and-forget by default (no per-packet ACK wait).\n"
              << "  * FIN always waits for ACKs (teardown is reliable).\n"
              << "  * --wait-acks enables waiting for ACKs after each DATA packet (restores stop-and-wait for DATA).\n";
}
...
static bool parse_args(int argc, char** argv, Args& a) {
    for (int i = 1; i < argc; ++i) {
        ...
        else if (s == "--wait-acks") a.wait_data_acks = true;
        else if (s == "--no-wait-acks") a.wait_data_acks = false;
...
private:
    ...
    void print_benchmark_summary() const {
        ...
        std::cout << "  data_acks: " << (A.wait_data_acks ? "on (stop-and-wait DATA)" : "off (fire-and-forget DATA)") << "\n";
        std::cout << "  fin_acks:  on (reliable FIN)\n";
    ...
    bool send_data(uint32_t seq, const std::vector<uint8_t>& app) {
        // Fast path: performance mode without per-packet ACK waiting (DATA is fire-and-forget).
        if (!A.wait_data_acks) {
            uint32_t ts = now_ms();
            std::vector<uint8_t> pkt(sizeof(RmHeader) + app.size());
            RmHeader h{};
            fill_header(h, seq, FLG_DATA, /*wnd*/1, ts, 0, /*retrans_id*/1);
            serialize_header(h, pkt.data());
    ...
    bool send_fin(uint32_t seq) {
        // FIN is always reliable: wait for ACKs from the whole cohort.
        constexpr uint8_t kMaxRetransId = 8;
