...
struct Args {
    ...
    // Performance mode:
    //  - If wait_acks == false (default), we do NOT wait for ACKs after each DATA/FIN send.
...
static bool parse_args(int argc, char** argv, Args& a) {
    for (int i = 1; i < argc; ++i) {
        ...
        else if (s == "--wait-acks") a.wait_acks = true;
        else if (s == "--no-wait-acks") a.wait_acks = false;
...
private:
    ...
    void print_benchmark_summary() const {
        ...
        std::cout << "  wait_acks: " << (A.wait_acks ? "on (reliable stop-and-wait)" : "off (fire-and-forget)") << "\n";
    ...
    bool send_data(uint32_t seq, const std::vector<uint8_t>& app) {
        // Fast path: performance mode without per-packet ACK waiting.
        if (!A.wait_acks) {
            uint32_t ts = now_ms();
    ...
    bool send_fin(uint32_t seq) {
        // Fast path: performance mode without waiting for FIN ACKs.
        if (!A.wait_acks) {
            uint32_t ts = now_ms();
            std::vector<uint8_t> pkt(sizeof(RmHeader));
            RmHeader h{};
            fill_header(h, seq, FLG_FIN, /*wnd*/0, ts, 0, /*retrans_id*/1);
            serialize_header(h, pkt.data());
            if (!xmit(pkt)) return false;
