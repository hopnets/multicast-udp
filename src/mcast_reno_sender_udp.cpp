...
// Multicast reliable sender â€” sliding-window TCP Reno (three-component design).
//
...
//   - handshake() : SYN â†’ cohort assembly â†’ START   (line-for-line copy)
//   - send_fin()  : FIN multicast â†’ wait_all_acks   (line-for-line copy)
...
//     - on_timeout()          : RTO timeout â†’ slow start.
//
...
//     - first_ack detection : signals main thread on ack_count 0 â†’ 1.
//     - T_agg timer         : dup-ACK contributions accepted only within tagg_ms
//                             of the first dup-ACK for a given slot.
//     - x% dupack threshold : fast retransmit when â‰¥ dupack_pct % of cohort
//                             have each sent â‰¥ 1 dup-ACK within T_agg.
...
// â”€â”€â”€ Protocol header (22 bytes) â€” identical to peel_sender â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#pragma pack(push, 1)
struct RmHeader {
...
struct Args {
    ...
    // â”€â”€ Multicast Reno tuning â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // Percentage of cohort receivers that must each send â‰¥1 dup-ACK within
...
// COMPONENT 1 â€” ACK Window (AckSlot + AckWindow)
//
// The sole per-packet state structure.  Covers the range [snd_una, snd_nxt).
// All access is under AckAggState::mtx.
//
// Design notes:
//  â€¢ first_ack_done     : set when ack_count goes 0 â†’ 1; used to emit a cwnd
//                         update exactly once per slot per epoch.
//  â€¢ dup_ack_senders    : set of unique peer_keys that sent â‰¥1 dup-ACK; size
//                         drives the x%-threshold check.
//  â€¢ dupack_window_open : T_agg timer guard.  Dup-ACK contributions accepted
//                         only between dupack_window_start and +tagg_ms.
//  â€¢ mark_retransmit()  : full slot reset â€” ack_count, first_ack_done, dup
//                         state â€” so the new epoch starts from scratch.
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

struct AckSlot {
    // â”€â”€ Forward-progress state â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    uint32_t ack_count      = 0;     // # receivers that ACKed past this seq
    bool     first_ack_done = false; // ack_count went 0â†’1 at least once

    // â”€â”€ Duplicate-ACK state â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // Unique peer_keys that sent â‰¥1 dup-ACK for this slot within T_agg.
    ...
    // â”€â”€ Epoch / retransmit state â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
struct AckWindow {
    ...
    //   - dup state cleared â†’ T_agg reopens fresh for the new epoch.
    void mark_retransmit(uint32_t seq, uint8_t new_retrans_id) {
        ...
        s.ack_count         = 0;     // reset: all receivers must re-ACK the new epoch
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// COMPONENT 2 â€” Reno State Machine (RenoSM)
//
...
struct RenoSM {
    ...
    // â”€â”€ Req 1: first ACK of a slot -> cwnd update (snd_una does not move) â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Req 6: exit fast recovery when all receivers ACKed past recovery_point â”€
    ...
    // â”€â”€ Req 5: x%-dupack threshold reached -> enter fast recovery â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Req 8/9: RTO timeout â†’ slow start (identical to standard Reno) â”€â”€â”€â”€â”€â”€â”€â”€
    void on_timeout() {
        ssthresh = std::max(cwnd / 2.0, 2.0);
        cwnd     = 1.0;
    ...
    // â”€â”€ Partial ACK during fast recovery (RFC 5681 Â§3.2 step 4) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    void on_partial_ack(uint32_t newly_acked) {
        ...
        cwnd += 1.0; // add back 1 SMSS per RFC 5681 Â§3.2 step 4
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// COMPONENT 3 â€” Shared state between main thread and ACK Aggregator thread
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
struct AckAggState {
    ...
    // â”€â”€ Per-receiver tracking (aggregator writes, main thread reads) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    std::unordered_map<uint64_t, uint32_t> peer_cum_ack; // peer_key â†’ rcv_nxt // TODO: do i need it? We are ditching per receiver tracking.. can't we do it directly in ack window?

    // Per-peer most-recently-advertised window (in packets).
    // Updated on every valid ACK (forward-progress and dup-ACK alike) so that
    // min_rwnd can INCREASE when a constrained receiver's buffer drains â€” unlike
    // a plain running-min which is monotonically non-increasing.
    std::unordered_map<uint64_t, uint32_t> peer_rwnd; // peer_key â†’ latest adv_wnd

    // â”€â”€ ACK Window (Component 1) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    AckWindow ack_wnd;

    // â”€â”€ Window base: all receivers confirmed everything below this â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    uint32_t committed_una = 1;

    // â”€â”€ Signals from aggregator to main thread â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    // Number of slots that went from ack_count=0 to ack_countâ‰¥1 since the
...
// â”€â”€â”€ In-flight metadata (main thread only) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Payload lives in all_segs[seq-1] â€” no copy needed.
...
// â”€â”€â”€ McastRenoSender â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
class McastRenoSender {
public:
    explicit McastRenoSender(const Args& args) : A(args) {}
    ~McastRenoSender() { if (fd >= 0) close(fd); }

    // â”€â”€ init â€” identical to PeelSender::init() â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    bool init() {
...
private:
    ...
    // HANDSHAKE â€” line-for-line copy of PeelSender::handshake()
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    bool handshake() {
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // FIN TEARDOWN â€” line-for-line copy of PeelSender
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // COMPONENT 3 â€” ACK AGGREGATOR THREAD
    //
    ...
    void ack_aggregator_loop() {
        ...
        while (!agg_stop.load()) {
            ...
            // â”€â”€ Receiver flow control: per-peer rwnd, recompute global min â”€â”€â”€â”€â”€â”€
            ...
            if (cum_ack > peer_prev) {
                // â”€â”€ Req 3: retrans_id filter (forward-progress path) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                // is from a stale epoch â€” discard.
                if (cum_ack > 0) {
                    const AckSlot* chk = agg.ack_wnd.get(cum_ack - 1);
                    if (chk && pkt_rid < chk->retrans_id) continue; // stale epoch
                }

                // â”€â”€ Req 1: credit ack_count; detect first-ACK events â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                // â”€â”€ Req 2: recompute committed_una = min(peer_cum_ack) â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                // â”€â”€ RTT sample â€” Karn's algorithm (fix) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                // â”€â”€ Duplicate ACK (cum_ack <= peer_prev) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                // â”€â”€ Req 3: retrans_id filter (dup-ACK path) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                // If the receiver's dup-ACK has pkt_rid < slot.retrans_id, it was
                // generated for an old epoch of this packet â€” discard.
                if (pkt_rid < slot->retrans_id) continue;

                // â”€â”€ Req 7: T_agg aggregation timer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                // â”€â”€ Req 5: record unique sender; check x% threshold â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    //
    // Implements req 1 (first-ACK â†’ cwnd, send more), req 2 (full-commit â†’
    ...
    bool transfer(uint32_t total) {
        ...
        RenoSM reno; // Reno State Machine â€” main thread only
        ...
        while (true) {
            ...
            // â”€â”€ Read current snd_una and receiver window â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            uint32_t committed;
            {
                std::lock_guard<std::mutex> lk(agg.mtx);
            ...
            // â”€â”€ Req 1 / 2: fill send window â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            // Effective window = min(cwnd, min_rwnd) â€” flow control from the most
            ...
            if (in_flight.empty()) break; // safety: committed check at loop top may lag one iteration

            // â”€â”€ RTO deadline: oldest in-flight segment â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            // â”€â”€ Wait: CV fires on first_ack, fast_retransmit, or committed â”€â”€â”€â”€
            ...
            // â”€â”€ RTT update (Karn's: tsecr set only for non-retransmit ACKs) â”€â”€â”€
            ...
            // â”€â”€ Req 1: first-ACK events â†’ cwnd grows, send more next iteration â”€
            ...
            if (first_ack_events > 0) {
                ...
                std::cerr << "  FIRST_ACK Ã—" << first_ack_events
                          << " cwnd=" << reno.cwnd
                          << " state=" << reno.state_str() << "\n";
                // Next loop iteration re-checks the window and sends new segments.
            }

            // â”€â”€ Req 2: committed_una advance â†’ snd_una moves â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            if (woke_up && new_committed > prev_committed) {
                ...
                // â”€â”€ Req 8: reset RTO backoff on committed_una advance â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                // â”€â”€ Req 6: exit fast recovery when ALL receivers ACKed past â”€â”€â”€
                ...
                if (reno.state == CCState::FAST_RECOVERY &&
                    ...
                    // Partial ACK (RFC 5681 Â§3.2 step 4): committed_una advanced
                    // but not past recovery_point â€” at least one loss still unresolved.
            ...
            // â”€â”€ Req 5: fast retransmit â†’ enter fast recovery â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            if (fast_retransmit) {
                ...
                if (in_flight.count(fast_retransmit_seq)) {
                    ...
                    std::cerr << "  FAST_RETRANSMIT seq=" << fast_retransmit_seq
                              << " already committed â€” skipping FR entry\n";
                }
            }

            // â”€â”€ Req 8/9: RTO timeout â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            if (!woke_up) {
                consec_timeouts++;
    ...
    // â”€â”€â”€ Send a new (first-time) data segment â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€â”€ Retransmit an in-flight segment â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // Payload is read directly from all_segs â€” no per-segment copy needed.
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // LOW-LEVEL HELPERS â€” identical to peel_sender
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
